...
PACKAGE=github.com/nholuongut/argo-events
...
$(GOPATH)/bin/golangci-lint:
	curl -sSfL https://raw.githubusercontent.com/golangci/golangci-lint/main/install.sh | sh -s -- -b `go env GOPATH`/bin v1.54.1
...
update-manifests-version:
	cat manifests/base/kustomization.yaml | sed 's/newTag: .*/newTag: $(VERSION)/' | sed 's@value: quay.io/nholuongut/argo-events:.*@value: quay.io/nholuongut/argo-events:$(VERSION)@' > /tmp/base_kustomization.yaml
