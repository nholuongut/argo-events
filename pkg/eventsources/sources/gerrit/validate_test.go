...
func TestValidateEventSource(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, "projects cannot be empty", err.Error())

	content, err := os.ReadFile(fmt.Sprintf("%s/%s", sources.EventSourceDir, "gerrit.yaml"))
	assert.Nil(t, err)

	var eventSource *v1alpha1.EventSource
	err = yaml.Unmarshal(content, &eventSource)
	assert.Nil(t, err)
	assert.NotNil(t, eventSource.Spec.Gerrit)

	for name, value := range eventSource.Spec.Gerrit {
		fmt.Println(name)
		l := &EventListener{
			GerritEventSource: value,
		}
