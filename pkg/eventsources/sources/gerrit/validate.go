...
func (listener *EventListener) ValidateEventSource(ctx context.Context) error {
	return validate(&listener.GerritEventSource)
}

func validate(eventSource *v1alpha1.GerritEventSource) error {
	if eventSource == nil {
		return v1alpha1.ErrNilEventSource
	}
	if len(eventSource.Projects) == 0 {
		return fmt.Errorf("projects cannot be empty")
	}
	if eventSource.Events == nil {
		return fmt.Errorf("events can't be empty")
	}
	if eventSource.GerritBaseURL == "" {
		return fmt.Errorf("gerrit base url can't be empty")
	}
